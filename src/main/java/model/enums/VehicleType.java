 ...
 *
 * <p>
 * Each vehicle type is bound to exactly one {@link CargoType} and has fixed
 * values for
 * speed (game units/sec before SPEED_SCALE division), cargo capacity,
 * per-game-minute
 * maintenance cost, and one-off purchase price.
 *
 * <p>
 * The two passenger types ({@code CITY_BUS}, {@code EXPRESS_BUS}) can deliver
 * to any
...
public enum VehicleType {
    ...
    public CargoType getAllowedCargo() {
        return allowedCargo;
    }

    /** Returns the display-name category string (e.g. "Wood", "Iron"). */
    public String getCategory() {
        return allowedCargo.displayName();
    }

    /**
     * Returns the movement speed in game units per second (before SPEED_SCALE
     * division).
     */
    public int getSpeed() {
        return speed;
    ...
    /** Returns the sell price (half of purchase price). */
    ...
    public boolean isPassenger() {
        return allowedCargo == CargoType.PASSENGERS;
    }
}
